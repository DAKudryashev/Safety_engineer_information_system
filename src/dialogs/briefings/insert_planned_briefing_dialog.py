...
class InsertPlannedDialog(QDialog):
    def __init__(self, engineers, documents, parent=None):
        ...
        self.responsible_combo.setCurrentIndex(0)  # Первый по умолчанию
        ...
        self.document_combo.setCurrentIndex(0)  # Пустое значение по умолчанию
    ...
    def validate_form(self):
        ...
        if not self.name_input.text().strip() or not self.content_input.text().strip():
            self.error_label.setText("Заполните все поля cо *!")
